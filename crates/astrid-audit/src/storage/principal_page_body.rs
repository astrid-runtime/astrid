...
while result.len() < limit {
    ...
    let keys = self
        .store
        .list_keys_with_prefix_page(NS_SESSION_ENTRIES, &prefix, cursor.as_deref(), page_limit)
        .await
    ...
    for key in &keys {
        ...
        let id = AuditEntryId(uuid::Uuid::parse_str(encoded_id)
            .map_err(|error| AuditError::StorageError(error.to_string()))?);
