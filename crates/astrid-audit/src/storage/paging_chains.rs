use super::{AuditError, AuditResult, KvAuditStorage, NS_CHAIN_METADATA};
use astrid_core::{PrincipalId, SessionId};

impl KvAuditStorage {
    pub(super) async fn load_session_chains_page(
        &self,
        session_id: &SessionId,
        after: Option<&str>,
        limit: usize,
    ) -> AuditResult<Vec<(String, Option<PrincipalId>)>> {
        let session_key = session_id.0.to_string();
        let prefix = format!("{session_key}:");
        let key_limit = if after.is_none() {
