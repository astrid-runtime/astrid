...
use astrid_storage::{
    Blake3ObjectIdentityV1, BulkIngestPolicy, ChunkingProfile, ContentChangeCache, ContentIngest,
    ContentName, KvQuotaResolver, NativeContentStagingArea, NativePrincipalContentStore,
    ObjectCacheCapacity, ObjectCacheConfig, ObjectCacheController, RuntimePrincipalStore,
    SourceEpoch, SourceFingerprint, SourceObservation, SourceScopeId, StableSourceId, StateOwner,
    StateOwnerCodecV1, open_runtime_principal_store,
...
async fn benchmark_bulk_ingest(
    config: &Config,
    root: &Path,
    source: &Path,
    source_digest: [u8; 32],
    ...
    report.record_bytes("astrid_bulk_reingest_unchanged", config.bytes, unchanged);
    report.record_bytes(
