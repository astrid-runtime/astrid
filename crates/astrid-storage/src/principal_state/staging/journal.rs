...
use crate::principal_state::native_io::{create_private_file, open_private_file_rw};
...
pub(super) fn open_journal(path: &Path) -> StorageResult<(File, JournalRecovery)> {
    let mut file = match std::fs::symlink_metadata(path) {
        Ok(_) => open_private_file_rw(path)?,
    ...
    while offset < file_len {
        ...
        if header[..8] != JOURNAL_MAGIC {
            if valid_frame_follows(file, offset, file_len)? {
                return Err(connection(format!(
                    "corrupt interior staging journal frame at {offset}: magic mismatch"
                )));
            }
            truncate_tail(file, offset)?;
            break;
        ...
        let frame_len = u64::try_from(JOURNAL_HEADER_BYTES)
            .map_err(|_| connection("staging journal header overflow".to_owned()))?
            .checked_add(payload_len)
            .ok_or_else(|| connection("staging journal frame length overflow".to_owned()))?;
        let frame_end = offset
            .checked_add(frame_len)
            .ok_or_else(|| connection("staging journal offset overflow".to_owned()))?;
        ...
        if frame_checksum(payload_len, &payload) != expected {
            if valid_frame_follows(file, offset, file_len)? {
                return Err(connection(format!(
                    "corrupt interior staging journal checksum at {offset}"
                )));
            }
            truncate_tail(file, offset)?;
            break;
        ...
        apply_record(
            decode_record(&payload).map_err(|detail| {
                connection(format!("decode staging journal at {offset}: {detail}"))
            })?,
            &mut pending,
            &mut completed,
        )?;
