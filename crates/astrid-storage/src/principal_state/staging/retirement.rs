...
use crate::principal_state::native_io::{PrivateDirectory, private_file_identity};
...
pub(super) fn create_marker_in(directory: &PrivateDirectory, key: StageKey) -> StorageResult<()> {
    let retired = PathBuf::from(retired_generation_name(key.sequence, key.id));
