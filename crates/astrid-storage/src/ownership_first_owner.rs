//! One-shot first-owner enrollment over the ownership graph.
//!
//! A pending claim is durable evidence of an authenticated request, not
//! authority. Only the commit CAS may publish the fleet membership and
//! principal ownership edge, so a crash before commit reopens as Pending with
//! no usable owner.
...
/// Durable enrollment state for the one-shot first-owner ceremony.
...
impl FirstOwnerEnrollment {
    /// Return the claim carried by a pending or enrolled state.
    #[must_use]
    pub const fn claim(self) -> Option<FirstOwnerClaim> {
        match self {
            Self::Unenrolled => None,
            Self::Pending { claim } | Self::Enrolled { claim } => Some(claim),
        }
    }
...
impl OwnershipSnapshot {
    pub(super) fn validate_first_owner(&self) -> Result<(), OwnershipError> {
        ...
        if let FirstOwnerEnrollment::Enrolled { claim } = enrollment {
            let Some(ownership) = self.principal_ownership.get(&claim.principal_uid()) else {
                return Err(OwnershipError::CorruptGraph(
                    "enrolled first-owner claim has no principal ownership edge".to_owned(),
                ));
            };
            if ownership.fleet_uid != claim.fleet_uid() || ownership.assigned_by != claim.user_uid()
            {
                return Err(OwnershipError::CorruptGraph(
                    "enrolled first-owner claim disagrees with principal ownership".to_owned(),
                ));
            }
...
impl super::OwnershipStore {
    ...
    /// # Errors
    ///
    /// Fails closed when the graph is unavailable, malformed, or violates an
    /// ownership invariant.
    pub async fn first_owner_state(&self) -> Result<FirstOwnerEnrollment, OwnershipError> {
        Ok(self.load().await?.first_owner_state())
    }

    /// Verify and durably record a signed first-owner request as Pending.
    ///
    /// This transition never writes a fleet membership or principal ownership
    /// edge. The authenticated kernel composition root must validate the
    /// machine/boot context before calling this storage operation.
    ///
    /// # Errors
    ///
    /// Returns a replay error for a different request and fails closed on
    /// malformed signatures or storage/invariant failures.
    pub async fn begin_first_owner(
        ...
        self.mutate(|graph| match graph.first_owner_state() {
            FirstOwnerEnrollment::Unenrolled => {
                graph.set_first_owner_state(&FirstOwnerEnrollment::Pending { claim });
                Ok(graph.first_owner_state())
            },
            FirstOwnerEnrollment::Pending { claim: existing }
            | FirstOwnerEnrollment::Enrolled { claim: existing }
                if existing == claim =>
            {
                Ok(graph.first_owner_state())
            },
            FirstOwnerEnrollment::Pending { .. } => Err(FirstOwnerError::Replay.into()),
            FirstOwnerEnrollment::Enrolled { .. } => Err(FirstOwnerError::AlreadyEnrolled.into()),
        })
        .await
    }
    ...
    ///
    /// The closure revalidates the live principal directory, deletion
    /// reservation, immutable user/fleet identities, owner membership, and
    /// unowned-principal condition inside the same optimistic CAS that writes
    /// the Enrolled record. A crash before that CAS leaves only Pending.
    ///
    /// # Errors
    ///
    /// Returns a replay or identity error when any bound fact differs, and
    /// fails closed on storage/invariant failures.
    ...
    fn apply_first_owner_commit(
        ...
        match graph.first_owner_state() {
            ...
            FirstOwnerEnrollment::Pending { .. } => {
                return Err(FirstOwnerError::Replay.into());
            },
            ...
            FirstOwnerEnrollment::Unenrolled => {
                return Err(FirstOwnerError::MissingPending.into());
            },
        }
