...
impl crate::content_dag::ContentSource for EngineContentSource<'_> {
...
fn contiguous_compaction_plan(
    ...
    let retention = CompactionRetention::new(
        ...
        crate::storage_model::RetentionPolicyId::new(engine.identify(&policy)),
        [
...
fn contiguous_copy_publishes_virtual_chunks_and_reopens() {
    ...
    assert_eq!(
        crate::content_dag::read_content(&EngineContentSource(&engine), descriptor.file())
            .unwrap(),
        content
    );
    ...
    assert_eq!(
        crate::content_dag::read_content(&EngineContentSource(&reopened), descriptor.file())
            .unwrap(),
        content
    );
}
...
fn represented_compaction_recovers_across_authority_and_blob_retirement() {
    for point in [
        ...
        assert_eq!(
            crate::content_dag::read_content(
