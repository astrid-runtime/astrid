...
use astrid_storage_content::{
    ...
    VerifiedContent, build_content, build_content_streaming, open_content,
    ...
    E: PrincipalProjectionEngine<P>,
{
    ...
    fn publish(
        ...
        loop {
            ...
            let transaction = self.encode_transaction(principal.clone(), header, built)?;
    ...
    pub fn delete(&self, principal: &P, name: &ContentName) -> Result<bool, PrincipalContentError> {
        loop {
            ...
            let transaction = self.encode_transaction(principal.clone(), header, None)?;
            match self.engine.commit_root(transaction) {
                Ok(_) => return Ok(true),
                Err(PrincipalProjectionError::Model(ModelError::RootConflict { .. })) => {},
                Err(error) => return Err(error.into()),
    ...
    pub fn list(&self, principal: &P) -> Result<Vec<ContentEntry>, PrincipalContentError> {
        self.header(principal).map(|header| header.catalog.list())
    }
...
impl ProjectionCachePayload for ContentHeader {
    fn retained_bytes(&self) -> u64 {
        let catalog_entries = self
            ...
            .entries
            .iter()
            .fold(0_usize, |total, (name, _)| {
                total
                    .saturating_add(std::mem::size_of::<(ContentName, CatalogValue)>())
        ...
        u64::try_from(
            std::mem::size_of::<Self>()
                .saturating_add(catalog_entries)
...
struct CachedVerifiedContent(VerifiedContent);
...
struct CachedPartialVerification(ContentVerificationState);
