...
pub(super) fn read_indexed_objects(
    arena: &File,
    ...
    while first < ordered.len() {
        ...
        for (expected_id, location) in &ordered[first..end] {
            ...
            let frame_len = frame_len(*location)?;
            let frame_len =
                usize::try_from(frame_len).map_err(|_| DurableError::EncodingOverflow)?;
            ...
            let frame = span
                ...
                .ok_or(DurableError::EncodingOverflow)?;
            let record = decode_indexed_frame(frame, *expected_id, *location, scheme, limits)?;
