...
#[derive(Debug)]
pub(super) struct RedirectAirlock;
...
///
/// Test-only: the production buffered path follows redirects manually (so each
/// hop re-runs the async security gate) and the streaming path uses
/// [`streaming_redirect_policy`]. This reqwest-level variant survives only as
/// the fixture behind the exempt-no-follow behavioural test.
#[cfg(test)]
...
/// Build the redirect policy for a request whose redirects are followed by
/// reqwest itself. Test-only fixture for the exempt-no-follow behavioural test.
///
...
/// redirect, so exempt requests refuse to follow any. The production streaming
/// path encodes the same exempt → `Policy::none()` rule inline.
#[cfg(test)]
pub(super) fn build_redirect_policy(
    exempt: bool,
...
/// Redirect policy for the streaming `follow` path: re-applies the airlock to
/// every hop, records a trip, and bounds by `max_redirects`.
pub(super) fn streaming_redirect_policy(
    tripped: Arc<AtomicBool>,
    max_redirects: usize,
) -> reqwest::redirect::Policy {
    reqwest::redirect::Policy::custom(move |attempt| {
        let prior = attempt.previous().len();
        if let Some(ip) = attempt.url().host_str().and_then(literal_ip)
            && !is_safe_ip(ip)
        {
            tripped.store(true, Ordering::Relaxed);
            return attempt.error(RedirectAirlock);
        }
        if prior >= max_redirects {
            attempt.stop()
        } else {
            attempt.follow()
        }
